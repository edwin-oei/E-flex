...
int runningBatteryLevel_percent = 0;
int currentBatteryLevel_percent = 0;
int dummyBatteryLevel_percent = 50;
long initialBatteryCharge_joules = 0;           // in Joules. Describes the current level of battery charge in Watt-hr. 1Wh = 3600J
long currentBatteryCharge_joules = 0;
...
void setup()                    // run once when the sketch starts
{
  ...
  delay(1500);    
  lcd.clear(); lcd.home();
  lcd.print("Renewables = "); lcd.print(renewables_watts);
  ...
  Serial.print("Initial battery charge = "); Serial.print(initialBatteryCharge_joules); Serial.println(" Joules");
...
int displayBatteryLevel_discharging(unsigned long startTime)
{
...
int displayBatteryLevel_charging(unsigned long startTime)
{
  int currentBatteryLevel_percent;
  currentBatteryCharge_joules = initialBatteryCharge_joules + (renewables_watts-total_system_power)/1000*((millis()-startTime)/1000);
  ...
  if (dummyBatteryLevel_percent == initialBatteryLevel_percent){
    ...
    lcd.setCursor(0, 1); 
    lcd.print(initialBatteryLevel_percent); lcd.print("%");
  }  
    ...
    if (currentBatteryLevel_percent < 100) {
      ...
      lcd.print("Battery level: ");
      lcd.setCursor(0, 1);
      lcd.print(currentBatteryLevel_percent); lcd.print("%");
      dummyBatteryLevel_percent = currentBatteryLevel_percent;
    }
    else {
      lcd.clear();
