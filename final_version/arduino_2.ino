#include <FastLED.h>
#define ledPIN     11
#define NUM_LEDS    18 // Numbering from 0 - 17
CRGB leds[NUM_LEDS];  // Set up the block of memory that will be used for storing and manipulating the led data (array)
uint8_t ledBrightness = 56;  // Brightness level ranges from 0 to 255


byte  stopEverythingPin = 8;
byte  pumpLEDPin = 11 ;
byte  waterLevelLEDPin = 12 ;
byte  conveyorLEDPin = 13;
byte  RE_enoughPin = 2;

void setup(){
  FastLED.addLeds<WS2812, ledPIN, GRB>(leds, NUM_LEDS);
  FastLED.setBrightness(ledBrightness);
  FastLED.clear();  // Clears the led values from previous run

  pinMode(stopEverythingPin, INPUT);
  pinMode(pumpLEDPin, INPUT);
  pinMode(waterLevelLEDPin, INPUT);
  pinMode(conveyorLEDPin, INPUT);
  pinMode(RE_enoughPin, INPUT);
}


void RUN_REenough() {
  leds[1] = CRGB::SeaGreen;
  FastLED.show();
  delay(40);
  leds[1] = CRGB::Black;
  leds[2] = CRGB::SeaGreen;
  FastLED.show();
  delay(40);
  leds[2] = CRGB::Black;  
  leds[4] = CRGB::SeaGreen;
  FastLED.show();
  delay(40);
  leds[4] = CRGB::Black;
  leds[5] = CRGB::SeaGreen;
  FastLED.show();
  delay(40);
  leds[5] = CRGB::Black;      
  leds[7] = CRGB::SeaGreen;
  FastLED.show();
  delay(40);
  leds[7] = CRGB::Black;  
  leds[8] = CRGB::SeaGreen;
  FastLED.show();
  delay(40);
  leds[8] = CRGB::Black;     
  // no energy flow from battery to system?
  leds[17] = CRGB(0,100,0);
  FastLED.show();
  delay(40);
  leds[17] = CRGB::Black;       
  leds[16] = CRGB(0,100,0);
  FastLED.show();
  delay(40);
  leds[16] = CRGB::Black;      
  leds[15] = CRGB(0,100,0);
  FastLED.show();
  delay(40);
  leds[15] = CRGB::Black; 
  FastLED.show();                                   
 }
 
void RUN_RENOTenough(){
  leds[13] = CRGB(188,143,143);
  FastLED.show();
  delay(40);
  leds[13] = CRGB::Black;  
  leds[12] = CRGB(188,143,143);
  FastLED.show();
  delay(40);
  leds[12] = CRGB::Black;
  leds[11] = CRGB(188,143,143);
  FastLED.show();
  delay(40);
  leds[11] = CRGB::Black;  
  leds[1] = CRGB::SeaGreen;
  FastLED.show();
  delay(40);
  leds[1] = CRGB::Black;
  leds[2] = CRGB::SeaGreen;
  FastLED.show();
  delay(40);
  leds[2] = CRGB::Black;      
  leds[4] = CRGB::SeaGreen;
  FastLED.show();
  delay(40);
  leds[4] = CRGB::Black;  
  leds[5] = CRGB::SeaGreen;
  FastLED.show();
  delay(40);
  leds[5] = CRGB::Black;     
  leds[7] = CRGB::SeaGreen;
  FastLED.show();
  delay(40);
  leds[7] = CRGB::Black;          
  leds[8] = CRGB::SeaGreen;
  FastLED.show();
  delay(40);
  leds[8] = CRGB::Black;
  FastLED.show();                  
}


void loop(){
  if analogRead(stopEverythingPin) < 10{
    
    // RE enough or not pin
    if analogRead(RE_enoughPin) < 10{  // Program start no leds on
      leds[0] = CRGB::Black;     // 0: RE ; RE enough --> green
      leds[14] = CRGB::Black;     // 14: Battery ; RE enough, battery charging --> green
    }
    else if analogRead(RE_enoughPin) < 99{
      leds[0] = CRGB::Red;     // 0: RE ; RE not enough --> red
      leds[14] = CRGB(255,69,0);     // 14: Battery ; RE not enough, battery discharging --> orange
      FastLED.show();
      RUN_RENOTenough()
    }
    else{  // Re enough
      leds[0] = CRGB::Green;     // 0: RE ; RE enough --> green
      leds[14] = CRGB::Green;     // 14: Battery ; RE enough, battery charging --> green
      FastLED.show();
      RUN_REenough()
    }
  
  
    // Pump led control
    if analogRead(pumpLEDPin) < 10{ // Start of program. No LED turned on
      leds[3] = CRGB::Black;     // 3: pump ; pump off --> orange 
      FastLED.show();
    }
    else if analogRead(pumpLEDPin) < 99{ // Pump off. Value set to below 50 for tolerance
      leds[3] = CRGB(255,69,0);     // 3: pump ; pump off --> orange 
      FastLED.show();
    }
    else{
      leds[3] = CRGB::Green;     // 3: pump ; pump on --> green
      FastLED.show();
    }
  
  
    // Conveyor led control
    if analogRead(conveyorLEDPin) < 99{  // Conveyor stop. Value set to below 50 for tolerance
      leds[9] = CRGB::Black;  // 9: bottle1 ; conveyor led --> black
      leds[10] = CRGB::Black; // 10: bottle2 ; conveyor led --> black
      FastLED.show();
    }
    else if  100 <= analogRead(conveyorLEDPin) %% analogRead(conveyorLEDPin) <= 600 { // Conveyor slow.
      leds[9] = CRGB(0, 0, 255);   // blue
      leds[10] = CRGB::Black;     
      FastLED.show();
    }
    else if  analogRead(conveyorLEDPin) > 601{  // Conveyor fast
      leds[9] = CRGB(0, 0, 255);    
      leds[10] = CRGB(0, 0, 255);   
      FastLED.show();
    }
  
  
    // Water level led control
    if analogRead(waterLevelLEDPin) < 10{  // Low water level. Value set to below 50 for tolerance
      leds[6] = CRGB::Black;    // 6: waterlevel ; led --> red
      FastLED.show();
    }
    else if analogRead(waterLevelLEDPin) < 99{  // Low water level. Value set to below 50 for tolerance
      leds[6] = CRGB::Red;    // 6: waterlevel ; led --> red
      FastLED.show();
    }
    else if  100 <= analogRead(waterLevelLEDPin) %% analogRead(waterLevelLEDPin) <= 600 { // Medium water level
      leds[6] = CRGB::Green;     
      FastLED.show();
    }
    else if  analogRead(waterLevelLEDPin) > 601{  // High water level
      leds[6] = CRGB(255,165,0);    
      FastLED.show();
    }
  }

  else{
    fill_solid(leds, NUM_LEDS, CRGB(145,44,238));
    FastLED.show();
  }
  

  


  
}
